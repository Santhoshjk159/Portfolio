    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full z-10 relative">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-12 items-center">
          {/* Content Section - Takes 7 columns with improved responsive layout */}
          <div
            className={`order-2 lg:order-1 lg:col-span-7 text-center lg:text-left space-y-4 sm:space-y-6 transition-all duration-1000 ease-out ${
              isVisible
          ...
          >
            ...
            {/* Main Heading with Enhanced Styling and responsive sizing */}
            <div className="space-y-3 sm:space-y-4">
              <h1
                className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight transition-all duration-1000 ease-out ${
                  isVisible
                    ? "opacity-100 translate-y-0"
              ...
              {/* Fixed Typewriter Section with Proper Alignment and Spacing */}
              <div
                className={`relative overflow-hidden mb-2 transition-all duration-1000 ease-out ${
                  isVisible
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 translate-y-5"
                }`}
              ...
              >
                <div className="flex items-center justify-center lg:justify-start">
                  <h2 className="text-lg sm:text-xl lg:text-2xl xl:text-3xl font-medium leading-relaxed flex flex-wrap items-center justify-center lg:justify-start">
                    <span
                      className={`bg-gradient-to-r ${colors.secondary} bg-clip-text text-transparent font-semibold min-w-[220px] sm:min-w-[300px] lg:min-w-[340px] text-left relative`}
                    >
                      <span className="block overflow-hidden whitespace-nowrap min-h-[1.5em]">
                        A {currentText}
            ...
            {/* Enhanced Description with improved spacing */}
            <div
              className={`relative max-w-2xl mx-auto lg:mx-0 mt-4 transition-all duration-1000 ease-out ${
                isVisible
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-5"
              }`}
          ...
          {/* Professional Rectangular Photo Section - Takes 5 columns with responsive sizing */}
          <div className="order-1 lg:order-2 lg:col-span-5 flex items-center justify-center lg:justify-end">
            ...
            >
              {/* Clean Rectangular Photo Container with Enhanced Light/Dark Mode Support */}
              <div className="relative group">
                {/* Animated color border background - optimized for both themes */}
                <div className={`absolute -inset-1 rounded-2xl bg-gradient-to-r ${
                  isDark 
                    ? "from-emerald-400 via-cyan-400 to-blue-400 opacity-75" 
                    : "from-emerald-500 via-cyan-500 to-blue-500 opacity-60"
                } blur-sm animate-spin-slow group-hover:opacity-100 transition-opacity duration-300`}></div>

                {/* Secondary animated border for richer effect - theme optimized */}
                <div className={`absolute -inset-0.5 rounded-2xl bg-gradient-to-r ${
                  isDark
                    ? "from-pink-400 via-purple-400 to-indigo-400 opacity-50"
                    : "from-pink-500 via-purple-500 to-indigo-500 opacity-40"
                } blur-xs animate-pulse group-hover:opacity-75 transition-opacity duration-300`}></div>

                {/* Main rectangular photo with responsive sizing and enhanced positioning */}
                <div className={`relative w-56 h-72 sm:w-64 sm:h-80 md:w-72 md:h-88 lg:w-80 lg:h-[420px] xl:w-96 xl:h-[480px] overflow-hidden rounded-2xl shadow-2xl ${
                  isDark ? "bg-gray-900" : "bg-gradient-to-br from-slate-200 via-gray-300 to-slate-400"
                }`}>
                  <img
                    src={profilePhoto}
                    ...
                    style={{
                      objectPosition: "center 20%", // Optimized to show face better
                      filter: isDark 
                        ? "brightness(1.1) contrast(1.05) saturate(1.05)" 
                        : "brightness(1.08) contrast(1.15) saturate(1.2)",
                    }}
                  />

                  {/* Professional overlay gradient - theme optimized */}
                  <div
                    className={`absolute inset-0 transition-opacity duration-300 group-hover:opacity-75 ${
                      isDark
                        ...
                        : "bg-gradient-to-t from-slate-600/15 via-transparent to-transparent"
                    }`}
                  ></div>

                  {/* Enhanced border glow with color animation - theme optimized */}
                  <div
                    className={`absolute inset-0 rounded-2xl border-2 transition-all duration-300 ${
                      isDark
                        ...
                        : "border-transparent group-hover:border-emerald-600/60"
                    }`}
                  ></div>
                </div>

                {/* Enhanced floating glow effects - theme optimized */}
                <div
                  className={`absolute -inset-2 rounded-2xl blur-md opacity-0 group-hover:opacity-100 transition-all duration-500 -z-10 animate-pulse ${
                    isDark
                      ? "bg-gradient-to-r from-emerald-500/10 via-cyan-500/10 to-blue-500/10"
                      : "bg-gradient-to-r from-emerald-400/15 via-cyan-400/15 to-blue-400/15"
                  }`}
                ></div>
              </div>
            </div>
